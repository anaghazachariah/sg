...
import pandas as pd 
import numpy as np
rcg_data_ = pd.read_csv("RCG_G0001.csv",skipinitialspace=True)

rcg_data_.fillna('missing', inplace=True)
...
for i in del_cols:
  del rcg_data_[i]
rcg_data_ = rcg_data_.add_suffix('_RCG')
rcg_data_.insert(0, 'ID', range(0, 0 + len(rcg_data_)))

rcg_cols=list(rcg_data_.columns) 

#READING MI
mi_data = pd.read_csv("MI_G0001.csv",skipinitialspace=True)
...
#MERGIN MI AND RCG
rcg_mi_join = pd.merge(rcg_data_, mi_data,  how='inner', left_on=['E2KACCT_NBR_RCG','BUS_UNIT_RCG','DEPT_ID_RCG','CCY_CD_RCG','ALTACCT_RCG','AFFIL_CD_RCG','GL_PROD_CD_RCG','OPERATING_UNIT_RCG','E2K_PRODUCT_CD_RCG'], right_on = ['ACCOUNT_MI','BUSINESS_UNIT_MI','DEPTID_MI','CURRENCY_MI','ALTACCT_MI','AFFILIATE_MI','OBJECT_CODE_MI','OPERATING_UNIT_MI','PRODUCT_MI'])
...
for i in range (0,len(rcg_data_)):
  if rcg_data_.loc[ i,'ORIG_CCY_MEAS_AMT_RCG']==0  :
    inventory_0_rows.append(rcg_data_.loc[ i,'ID'])
...
for i in range(0,2105):
  if i not in rows_present_in_rcg_mi:
    rows_not_present_in_rcg_mi.append(rcg_data_.loc[ i,'ID'])
...
for i in wrong_glaam:#deleting records from rcg
  delete_row = rcg_data_[rcg_data_["ID"]==i].index
  rcg_data_ = rcg_data_.drop(delete_row)
rcg_data = rcg_data_.reset_index(drop=True) 
...
rcg_mi_e2k_merged = pd.merge(  rcg_mi_merged,e2k_data, how='left', left_on=['E2KACCT_NBR_RCG'], right_on = ['E2KACCOUNTMNEMONIC_E2K'])
...
rcg_mi_e2k_merged_ = rcg_mi_e2k_merged.drop(delete_row)
rcg_mi_e2k_merged = rcg_mi_e2k_merged_.reset_index(drop=True) 
...
#OPENING GRIT_E2K
grit_e2k_data = pd.read_csv("GRIT_E2k_PCI_G0001.csv",skipinitialspace=True)
...
for i in del_cols:
  del grit_e2k_data[i]
grit_e2k_data_new=grit_e2k_data.drop_duplicates()

#OPENING GRIT_PRC
grit_prc_data = pd.read_csv("GRIT_PRC_ADJ_G0001.csv",skipinitialspace=True)
grit_prc_data.fillna('missing', inplace=True)
grit_prc_data = grit_prc_data.add_suffix('_GRIT_PRC')
grit_prc_data=grit_prc_data.drop_duplicates()

#GRIT AND PRC MERGE
grit_e2k_data = grit_e2k_data_new.reset_index(drop=True) 
...
empty=[]
for k in range(0,len(grit_e2k_enitiy_list)):
  empty.append('missing')
grit_e2k_data['PRC_GRIT_PRC'] = empty
grit_e2k_data['CHAPTER_GRIT_PRC'] = empty
grit_e2k_data['GAAP_TYPE_GRIT_PRC'] = empty
grit_e2k_data['TRAN_AMOUNT_ADJ_GRIT_PRC'] = empty
grit_e2k_data['TOTAL_AFTER_ADJ_GRIT_PRC'] = empty
grit_e2k_data['ADJUSTMENT_TYPE_GRIT_PRC'] = empty 	
...
infofy_data=infofy_data.drop_duplicates()
grit_e2k_datas=grit_e2k_data.copy()
grit_e2k_datas = grit_e2k_datas.reset_index(drop=True) 
grit_e2k_enitiy_list=grit_e2k_datas['ENTITY_GRIT_E2k'].tolist()
grit_e2k_counterparty_list=grit_e2k_datas['COUNTERPART_GRIT_E2k'].tolist()
grit_e2k_prc_ias_list=grit_e2k_datas['PRC_IAS_GRIT_E2k'].tolist()
grit_e2k_prc_fr_list=grit_e2k_datas['PRC_FR_GRIT_E2k'].tolist()
grit_e2k_chap_fr_list=grit_e2k_datas['CHPFR_GRIT_E2k'].tolist()
grit_e2k_crncy_list=grit_e2k_datas['CURRENCY_GRIT_E2k'].tolist()
grit_e2k_chap_ias_list=grit_e2k_datas['CHPIAS_GRIT_E2k'].tolist()
...
infofy_crncy_list=infofy_data['TRAN_CURRENCY_INFOFY'].tolist()
empty=[]
for k in range(0,len(grit_e2k_datas)):
  empty.append('missing')

grit_e2k_datas['ORIG_PRC_INFOFY'] = empty
grit_e2k_datas['RECLASS_PRC_INFOFY'] = empty
grit_e2k_datas['ORIG_CHAPTER_INFOFY'] = empty
grit_e2k_datas['RECLASS_CHAPTER_INFOFY'] = empty
grit_e2k_datas['ORIG_PRC_MNEMONIC_INFOFY'] = empty
grit_e2k_datas['RECLASS_PRC_MNEMONIC_INFOFY'] = empty
grit_e2k_datas['RIG_ASSET_LIAB_FLAG_INFOFY'] = empty
grit_e2k_datas['RECLASS_ASSET_LIAB_FLAG_INFOFY'] = empty
ind=0
c=0
for j in range(0,len(grit_e2k_enitiy_list)):
  ...
  for i in range(0,len(infofy_chap_list)):
    if infofy_entity_list[i]==grit_e2k_enitiy_list[j] and infofy_counterparty_list[i]==grit_e2k_counterparty_list[j] and infofy_crncy_list[i]==grit_e2k_crncy_list[j]:
      ...
      if str(q1)==str(q2) or str(q1)==str(q3):
        ...
        w3=grit_e2k_chap_ias_list[j]
        if (w1==w2) or (w1==w3):
          print('entity'+str(infofy_entity_list[i])+'\tcounterparty'+str(infofy_counterparty_list[i])+'\tcurrency'+str(infofy_crncy_list[i])+'\tinfofy_prc'+str(infofy_prc_list[i])+'\tinfofy_chapter'+str(infofy_chap_list[i]))
          c=c+1
          grit_e2k_datas.at[ind,'ORIG_PRC_INFOFY']=str(infofy_data.loc[i,'ORIG_PRC_INFOFY'])
         
          grit_e2k_datas.at[ind,'RECLASS_PRC_INFOFY']=str(infofy_data.loc[i,'RECLASS_PRC_INFOFY'])

          grit_e2k_datas.at[ind,'ORIG_CHAPTER_INFOFY']=str(infofy_data.loc[i,'ORIG_CHAPTER_INFOFY'])
          grit_e2k_datas.at[ind,'RECLASS_CHAPTER_INFOFY']=str(infofy_data.loc[i,'RECLASS_CHAPTER_INFOFY'])

          grit_e2k_datas.at[ind,'ORIG_PRC_MNEMONIC_INFOFY']=str(infofy_data.loc[i,'ORIG_PRC_MNEMONIC_INFOFY'])
          grit_e2k_datas.at[ind,'RECLASS_PRC_MNEMONIC_INFOFY']=str(infofy_data.loc[i,'RECLASS_PRC_MNEMONIC_INFOFY'])
          
          grit_e2k_datas.at[ind,'RIG_ASSET_LIAB_FLAG_INFOFY']=str(infofy_data.loc[i,'ORIG_ASSET_LIAB_FLAG_INFOFY'])
          grit_e2k_datas.at[ind,'RECLASS_ASSET_LIAB_FLAG_INFOFY']=str(infofy_data.loc[i,'RECLASS_ASSET_LIAB_FLAG_INFOFY'])
         
       
  ind=ind+1'''
#RCG,MI,GRIT,E2K,PRC,INFOFY JOIN
#rcg_grit_e2k_prc_infofy_join = pd.merge(rcg_mi_e2k_prc_merged, grit_e2k_datas,  how='left', left_on=['E2KACCT_NBR_RCG','BUS_UNIT_RCG','DEPT_ID_RCG','CCY_CD_RCG','ALTACCT_RCG','AFFIL_CD_RCG'], right_on = ['ACCOUNT_GRIT_E2k','BUSINESS_UNIT_GRIT_E2k','DEPTID_GRIT_E2k','CURRENCY_GRIT_E2k','ALTACCT_GRIT_E2k','AFFILIATE_GRIT_E2k'])
#uncomment above line if we want to include infofy
rcg_grit_e2k_prc_infofy_join = pd.merge(rcg_mi_e2k_prc_merged, grit_e2k_data,  how='left', left_on=['E2KACCT_NBR_RCG','BUS_UNIT_RCG','DEPT_ID_RCG','CCY_CD_RCG','ALTACCT_RCG','AFFIL_CD_RCG'], right_on = ['ACCOUNT_GRIT_E2k','BUSINESS_UNIT_GRIT_E2k','DEPTID_GRIT_E2k','CURRENCY_GRIT_E2k','ALTACCT_GRIT_E2k','AFFILIATE_GRIT_E2k'])
...
rcg_grit_e2k_prc_infofy_join .to_csv('G0001_grit_rcg_mi_e2k_prc.csv') 
